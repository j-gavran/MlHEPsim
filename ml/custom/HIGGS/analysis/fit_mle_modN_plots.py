...
class FitMLE(FitSetup):
    ...
    def plot_poi_money(self, label="", clip_yerr=1.2, mc_file="", ml_file=""):
        ...
        yerr = bestfits_ML["mu_err"].to_numpy()
        try:
    ...
    def plot_fit(self, lumi_idx=0, ylim=(0.5, 1.5), top_ylim=None, postfit=True, label=""):
        ...
        if self.cut_variable:
            ...
            hep_plot.ax.set_xlabel("classifier score")
...
if __name__ == "__main__":
    ...
    if run:
        ...
        mle_fit.plot_poi(label=f"at {sig_frac*100}% sig fraction", save_file="mle_mc_only_Bmod.p")
        # mle_fit.plot_poi_money(
        #     label=f"at {sig_frac*100}% sig fraction", mc_file="mle_mc_only_N.p", ml_file="mle_Bmod.p"
        # )
        # mle_fit.plot_poi_money(
        #     label=f"at {sig_frac*100}% sig fraction", mc_file="mle_mc_only_Bmod.p", ml_file="mle_B.p"
        # )

        mle_fit.plot_poi_money(
            label=f"at {sig_frac*100}% sig fraction", mc_file="mle_mc_only_cut_m_bb_Bmod.p", ml_file="mle_cut_m_bb_B.p"
        )
