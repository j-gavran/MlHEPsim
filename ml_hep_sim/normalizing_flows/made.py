...
class ResMaskedLinear(nn.Module):
    def __init__(self, k, mask, activation, n=2, zero_initialization=True, batchnorm=True):
        """
        References
        ----------
        ...
        for _ in range(n):
            if batchnorm:
                layers.append(nn.BatchNorm1d(k, eps=1e-3))
...
class BaseResMADE(BaseMADE):
    def __init__(self, input_size, k, n_blocks, l=2, activation="ReLU", res_batchnorm=False, **kwargs):
        """MADE with residual connections.

        Note
        ----
        Input order is assumed sequential.
        ...
        for _ in range(n_blocks):
            self.blocks.append(ResMaskedLinear(k, hidden_layer_mask, activation_fn, l, batchnorm=res_batchnorm))
...
if __name__ == "__main__":
    from ml_hep_sim.ml_utils import netron_model
