...
class HistMaker:
    ...
    def setup(
        ...
        if not self.mc_only:
            ...
            logging.info(
                f"Splits ML bkg-to-data eff={eff}, all={lbkg} bkg={N_mc_bkg_cut} data_bkg={N_mc_bkg_data_cut} "
            )
        if self.mc_only:
            ...
            lbkg = len(events["bkg"])
            eff = lbkg / (self.N_gen_bkg + self.N_mc_bkg)
            N_gen_bkg_cut = int(self.N_gen_bkg * eff)
            N_mc_bkg_cut = lbkg - N_gen_bkg_cut

            events["bkg"], events["bkg_data"] = (
                events["bkg"][:N_gen_bkg_cut],
                events["bkg"][N_gen_bkg_cut : N_gen_bkg_cut + N_mc_bkg_cut],
            )  # asimov bkg N and events
            events[self.model_name] = events["bkg"]  # replace ML with MC

            logging.info(f"Splits MC bkg-to-data eff={eff}, all={lbkg} bkg={N_gen_bkg_cut} data_bkg={N_mc_bkg_cut} ")
    ...
    def plot_histograms(self, ylim=(0.5, 1.5), top_ylim=None, plot_separate_sig=False, weighted=True):
        ...
        save_str += "_NB_BKP"  # BPK extra label
