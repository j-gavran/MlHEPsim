...
from ml_hep_sim.pipeline.distributed_pipes import Block
...
class ConfigBuilderBlock(Block):
    ...
    def run(self, *args):
        ...
        if self.model_name == "none":
            logging.warning("Model name is set to 'none'!")
...
class ModelBuilderBlock(Block):
    ...
    def run(self, *args, **model_kwargs):
        """Build the model."""
        self.setup(*args)
...
class GeneratedDataVerifierBlock(Block):
    ...
    @staticmethod
    def _check_invalid(ar, msg="running data check"):
        check = {
            "nan": np.isnan(ar),
...
class PCARunnerBlock(Block):
    ...
    def run(self, *args, remove_nans=True):
        self.setup(*args)
...
class StatTestRunnerBlock(Block):
    ...
    def run(self, *args, n_bins="auto", use_r=False, bin_range=None):
        """Run tests on data."""
        self.setup(*args)
    ...
    def run(self, *args):
        ...
        for _ in range(self.N):
            try:
                ...
                logging.warning("DataLoader iterated through the whole dataset! Decrease N.")
                raise StopIteration
